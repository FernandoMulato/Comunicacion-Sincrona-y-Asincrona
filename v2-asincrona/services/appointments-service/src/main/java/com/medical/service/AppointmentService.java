 ...
 * Now uses async RabbitMQ communication for patient validation.
 */
@Service
...
public class AppointmentService {
     ...
     * Uses async RabbitMQ validation.
     */
    @Transactional
    public AppointmentResponse createAppointment(CreateAppointmentRequest request) {
        // Validate patient exists via async RabbitMQ call to users-service
        var validationResponse = patientValidationClient.validatePatient(request.getPatientDocument());
        
        if (!validationResponse.isValid()) {
            throw new IllegalArgumentException(
                validationResponse.getError() != null 
                    ? validationResponse.getError() 
                    : "Patient not found with document: " + request.getPatientDocument()
            );
        }
        ...
        Appointment appointment = Appointment.builder()
                ...
                .patientName(request.getPatientName() != null ? request.getPatientName() : validationResponse.getPatientName())
